...
# read file
normal_wxs <- fread("processed_CombinedVariants_filtered_WXS_normal.txt",
                    stringsAsFactors=FALSE, strip.white=TRUE, check.names=FALSE, header=TRUE, fill=TRUE)
tumor_wxs <-  fread("processed_CombinedVariants_filtered_WXS_tumor.txt",
                    stringsAsFactors=FALSE, strip.white=TRUE, check.names=FALSE, header=TRUE, fill=TRUE)
tumor_rnaseq <- fread("processed_CombinedVariants_filtered_rnaseq_tumor.txt",
                    stringsAsFactors=FALSE, strip.white=TRUE, check.names=FALSE, header=TRUE, fill=TRUE)                    
...
print(paste0("total variants in combined wxs and rnaseq", " ", dim(combined_variants)))
...
# merge patients info 
normal_sample <- fread("sample_WXS_normal.txt", stringsAsFactors=FALSE, strip.white=TRUE, check.names=FALSE, header=TRUE, fill=TRUE)  
normal_sample <- as.data.frame(normal_sample)

tumor_sample <- fread("sample_WXS_tumor.txt",stringsAsFactors=FALSE, strip.white=TRUE, check.names=FALSE, header=TRUE, fill=TRUE)  
tumor_sample <- as.data.frame(tumor_sample)

rnaseq_sample <- fread("sample_rnaseq.txt", stringsAsFactors=FALSE, strip.white=TRUE, check.names=FALSE, header=TRUE, fill=TRUE)  
...
fwrite(final_table, paste0(outpath,"combined_normal_unknown_filled.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)  
...
fwrite(final_table_colarrange, paste0(outpath,"combined_normal_unknown_filled_arranged.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)  
...
# check the values across all columns are same
for(i in 1:length(mynames)){
	df <- final_table_colarrange[, colnames(final_table_colarrange)[grepl(mynames[i], names(final_table_colarrange))]]
    print(all(sapply(df, identical, df[,1])))
}
...
variantsForAnnovar$end <- variantsForAnnovar$POS
variantsForAnnovar <- variantsForAnnovar[, c("CHROM", "POS", "end", "REF", "ALT")]
fwrite(variantsForAnnovar, paste0(outpath, "final_variantsForAnnovar.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
