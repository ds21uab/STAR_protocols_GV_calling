...
# this script requires: combinedVariantsFromAllSamples.txt file as input, header.txt and output path where you want to save the processed file
...
# set the output path
output_path <- "/pathToSaveTheProcessedFile/" #provide the directory path where you want to save the processed file
...
# change colnames
segment_header <- fread(paste0("pathToDir/header.txt"),
  stringsAsFactors=FALSE,
  strip.white=TRUE,
  check.names=FALSE,
  header=FALSE)
...
fwrite(merged_samples_with_mutstatus, paste0(output_path, "processed_CombinedVariantsSegment_", output_name, ".txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
